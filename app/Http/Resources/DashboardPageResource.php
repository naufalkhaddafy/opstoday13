...
class DashboardPageResource extends JsonResource
{
    ...
    public function toArray(Request $request): array
    {
        ...
        foreach ($users as $user) {
            ...
            if ($activeLeave) {
                $status = $activeLeave->type; // cuti, sakit, izin
        ...
        // Sort by status: Hadir/Tidak Lengkap first, then Cuti/Sakit/Izin, then Absen, then Off Day
        usort($employeeStatuses, function ($a, $b) {
            $order = [
                'hadir' => 1,
                'tidak_lengkap' => 2,
                'sakit' => 3,
