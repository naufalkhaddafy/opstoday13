            ...
            if (log.early_leave_minutes > 0) {
                details.push(
                    <span key="early" className="text-xs text-amber-600 dark:text-amber-400 font-medium">
                        Pulang Cepat {log.early_leave_minutes}m
                    </span>
                );
            }
            if (log.overtime_minutes > 0) {
                details.push(
                    <span key="overtime" className="text-xs text-blue-600 dark:text-blue-400 font-medium">
                        Lembur {log.overtime_minutes}m
                    </span>
                );
            }
        }
        
        if (details.length === 0 && (log.presence_status === 'hadir' || log.presence_status === 'tidak_lengkap')) {
            return <span className="text-xs text-emerald-600 dark:text-emerald-400 font-medium">Tepat Waktu</span>;
        }
            ...
            <div className="flex h-full flex-1 flex-col gap-6 p-4 max-w-7xl mx-auto w-full">
                    ...
                    <div className="lg:col-span-2 flex flex-col justify-between p-6 rounded-xl border bg-card text-card-foreground shadow-sm bg-gradient-to-br from-indigo-50/40 via-transparent to-transparent dark:from-indigo-950/10">
                        <div>
                                ...
                                <div className="flex items-center gap-3">
                                    ...
                                    <div>
                                        ...
                                        <p className="text-xs text-muted-foreground mt-0.5">
                                            {user.email} {user.employee_id && `• ID: ${user.employee_id}`}
                                        </p>
                                    </div>
                                </div>
                            ...
                            <p className="text-muted-foreground mt-4 text-sm max-w-xl">
                                Memantau rekap absensi karyawan bulanan, log clock-in/out harian dari mesin fingerprint, dan persentase KPI kehadiran karyawan terkait.
                            </p>
                        </div>
                        <div className="flex items-center gap-2 mt-6 border-t pt-4">
                            ...
                            <span className="text-xs text-muted-foreground font-medium">Data terintegrasi real-time</span>
                        </div>
                    </div>

                    <Card className="border-indigo-100 dark:border-indigo-950/60 bg-gradient-to-br from-indigo-50/20 to-transparent dark:from-indigo-950/5">
                        <CardHeader className="pb-2">
                            <CardTitle className="text-sm font-medium flex items-center text-muted-foreground">
                                <CalendarClock className="mr-2 h-4 w-4 text-indigo-500" /> Shift Kerja Aktif
                            </CardTitle>
                        </CardHeader>
                        <CardContent>
                            {current_shift ? (
                                <div className="space-y-3">
                                    <div>
                                        <div className="text-lg font-bold text-foreground">{current_shift.name} ({current_shift.code})</div>
                                    ...
                                    <div className="flex items-center gap-4 pt-2 border-t text-sm">
                                        <div className="flex items-center gap-1.5 text-foreground">
                                            ...
                                            <span>{current_shift.start_time} - {current_shift.end_time}</span>
                                        </div>
                                    </div>
                                </div>
                            ) : (
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-4">
                    ...
                    <Card className="hover:border-foreground/20 transition-colors">
                        ...
                        <CardContent>
                            ...
                            <div className="text-xs text-muted-foreground mt-2 flex items-center gap-1 font-medium">
                                ...
                                {summary.total_absent > 0 ? 'Terdapat ketidakhadiran tanpa keterangan' : 'Karyawan nihil Alpha bulan ini!'}
                            </div>
                        </CardContent>
                    </Card>
                    ...
                    <Card className="hover:border-foreground/20 transition-colors">
                        <CardHeader className="pb-2">
                            <CardDescription className="text-[10px] uppercase font-bold tracking-wider text-muted-foreground flex items-center justify-between">
                                Kehadiran Tidak Lengkap <TrendingUp className="h-3.5 w-3.5 text-blue-500" />
                            </CardDescription>
                        </CardHeader>
                        <CardContent>
                            <div className="flex items-baseline gap-2">
                                <span className="text-3xl font-bold tracking-tight text-foreground">{summary.total_incomplete}</span>
